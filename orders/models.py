...
class OrderItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product_id:
            ...
            Product = apps.get_model(self._meta.app_label, "Product")
            product = getattr(self, "product", None) or Product.objects.only("price").filter(pk=self.product_id).first()
            if product:
...
class CombinedOrder(models.Model):
    ...
    def summarized_items_by_category(self):
        ...
        orders_qs = self.orders.select_related("account__participant__program").prefetch_related("items__product__category")
