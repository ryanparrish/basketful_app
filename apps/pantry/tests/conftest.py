...
def account_fixture(participant_fixture):
    ...
    """
    from apps.log.tests.test_logging import log_vouchers_for_account  # lazy import
...
def meat_product_with_manager(db):
    ...
    # ProductLimit with a low limit to trigger validation
    product_limit = ProductLimitFactory(
    ...
    meat_product = ProductFactory(
        name="Ground Beef",
        category=meat_category,
        weight_lbs=Decimal("5.0"),  # per unit weight
        price=Decimal("10.00"),
    )
